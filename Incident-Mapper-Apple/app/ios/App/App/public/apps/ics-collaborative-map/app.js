(() => {
  ...
  async function exportMapPdf() {
    ...
    const bounds = getMapExportBounds();
    if (!bounds || !bounds.isValid()) {
      setStatus("Place one or more map items before exporting PDF.");
      return;
    }
    setStatus("Fitting map for PDF export…");
    const previousCenter = state.map.getCenter();
    const previousZoom = state.map.getZoom();
    const previousMapStyleTrayOpen = state.mapStyleTrayOpen;
    const previousWeatherPanelOpen = state.weatherPanelOpen;
    ...
    try {
      state.map.invalidateSize({ pan: false, animate: false });
      state.map.fitBounds(bounds.pad(0.04), { padding: [12, 12], maxZoom: 19, animate: false });
      await waitForMapMoveEnd();
      await waitForActiveTilesLoaded();
      document.body.classList.add("print-map-live");
      setStatus("Print dialog open — choose 'Save as PDF'.");
      const cleanup = () => {
        document.body.classList.remove("print-map-live");
        window.removeEventListener("afterprint", cleanup);
      };
      window.addEventListener("afterprint", cleanup);
      window.print();
