...
actor MockHazmatRepository: HazmatRepository {
    ...
    func fetchTrackingReview(for sessionID: UUID) async throws -> SessionTrackingReview {
        ...
        let points = try await fetchTrackingPoints(for: scenario.scenarioName)
        let participantNames = Array(Set(points.map(\.traineeID))).sorted()

        return SessionTrackingReview(
            sessionID: sessionID,
            participants: participantNames.map { name in
                let latest = points.filter { $0.traineeID == name }.max(by: { $0.createdAt < $1.createdAt })
                return SessionTrackingParticipant(
                    id: UUID(),
                    traineeName: name,
                    deviceType: .airMonitor,
                    joinedAt: .now.addingTimeInterval(-900),
                    lastSeenAt: latest?.createdAt,
                    latestPoint: latest.map {
                        SessionTrackingParticipant.LatestPoint(
                            recordedAt: $0.createdAt,
                            receivedAt: $0.createdAt,
                            latitude: $0.latitude,
