...
struct MetricTile: View {
    ...
    var body: some View {
        ...
        .id("\(title)-tile-\(value)-\(alarmState)")
        ...
        .onChange(of: alarmState) { oldValue, newValue in
            updatePulseAnimation()
        }
    }
    ...
    private var valueColor: Color {
        // Keep metric value high-contrast in all alarm states.
        .white
    }

    private var labelColor: Color {
        alarmState == .normal ? .white.opacity(0.72) : .white.opacity(0.9)
    }

    private var unitColor: Color {
        alarmState == .normal ? .white.opacity(0.72) : .white.opacity(0.88)
    }
