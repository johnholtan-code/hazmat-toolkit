    ...
    var body: some View {
        ScreenShell(title: model.currentAirMonitorDisplayName, subtitle: model.selectedMonitor?.rawValue ?? "Gas Monitor") {
            GeometryReader { proxy in
                ...
                let calibrationActive = model.airMonitorCalibrationStep != nil
                ...
                ZStack {
                    ScrollView {
                        VStack(spacing: 14) {
                            if let scenario = model.selectedScenario {
                                ...
                                if model.showsSamplingFallbackNotice {
                                    Text("Sampling adjustments are not configured for this zone. Using device fallback values for HIGH/LOW position.")
                            ...
                            LazyVGrid(columns: columns, alignment: .leading, spacing: 10) {
                                ForEach(model.airMonitorSensors) { sensor in
                                    MetricTile(
                                        title: sensor.catalogAbbr,
                                        value: {
                                            model.airMonitorValueText(for: sensor)
                                        }(),
                    ...
                    .blur(radius: calibrationActive ? 5 : 0)
                    .allowsHitTesting(!calibrationActive)
