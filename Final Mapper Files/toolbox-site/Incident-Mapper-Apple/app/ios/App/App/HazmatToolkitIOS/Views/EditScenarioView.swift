...
struct EditScenarioView: View {
            ...
            Section("Map / Shape Editing") {
                if variant == .radiation {
                    ...
                    Text(isPlacingRadiationPin
                         ? "Tap the map to place a radiation pin. The Radiation sheet opens immediately so you can set dose, background, and shielding."
                        ...
                        Task {
                            ...
                            if let editingID = editingShapeID {
                                await store.updateRadiationPinShape(
                                    ...
                                    background: radiationBackground,
    ...
    private var radiationEditorPins: [MapPinItem] {
        var pins: [MapPinItem] = shapes.compactMap { shape in
            guard let coordinate = radiationCoordinate(for: shape) else { return nil }
        ...
        if let pendingRadiationCoordinate {
            pins.append(
                MapPinItem(
